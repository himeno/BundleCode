...
['reh', 'reh-web'].forEach(type => {
	...
	BUILD_TARGETS.forEach(buildTarget => {
		...
		['', 'min'].forEach(minified => {
			...
			// BundleCode: mangling costs half an hour of rename resolution across
			// 12k classes and only buys a smaller download. The server is built by
			// hand here rather than by CI, so the wait is not worth it. Minification
			// still runs, so the artifact stays a reasonable size.
