...
/**
 * Whether the file is a Windows binary. PE images start with `MZ`.
 *
 * Needed because `**\/*.node` also matches prebuilds for other platforms, and
 * rcedit cannot open those.
 */
async function isPortableExecutable(filePath: string): Promise<boolean> {
	...
	try {
		...
		return bytesRead === 2 && magic[0] === 0x4d && magic[1] === 0x5a;
	} finally {
		await handle.close();
	}
}
...
function patchWin32DependenciesTask(destinationFolderName: string) {
	...
	return async () => {
		...
		// **`**\/*.node` は他プラットフォームの prebuild も拾う。** 拡張機能には全 OS 分の
		// バイナリを同梱しているパッケージがあり（`@anthropic-ai/claude-agent-sdk` の
		// `vendor/audio-capture/arm64-linux/audio-capture.node` で実際に踏んだ）、
		// rcedit はそれを開けずに終了コード 1 で落ちる——**パッケージングが全部
		// 終わった後に**。混ざっていること自体は異常ではないので、除くのはこちら側の仕事。
		//
		// パスで除外すると次に別のパッケージが増えたときにまた同じ所で止まるので、
		// 中身で見る。
		const patchable = (await Promise.all(
			deps.map(async dep => await isPortableExecutable(path.join(cwd, dep)) ? dep : undefined)
		...
		const patchPromises = patchable.map<Promise<unknown>>(async dep => {
