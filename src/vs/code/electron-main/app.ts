...
import { BundleHost } from '../../platform/windows/electron-main/bundleHost.js';
...
export class CodeApplication extends Disposable {
	...
	private configureSession(): void {
		...
		const isAllowedVsCodeFileRequest = (details: Electron.OnBeforeRequestListenerDetails) => {
			...
			// Workbenches hosted as views inside a shared window have no window
			// of their own, so they have to be matched separately
			for (const webContents of BundleHost.getAllTabWebContents()) {
				if (!webContents.isDestroyed() && frame.processId === webContents.mainFrame.processId) {
					return true;
				}
			}
