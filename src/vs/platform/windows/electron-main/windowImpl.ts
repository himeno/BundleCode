...
/**
 * Hosting every workspace as a view inside one native window is what BundleCode
 * exists for, so it is on unless explicitly disabled. Env driven because it has
 * to be decided before any configuration service is available to the window.
 */
function useBundledTabs(): boolean {
	return process.env['VSCODE_BUNDLE_TABS'] !== '0';
}
...
export class CodeWindow extends BaseWindow implements ICodeWindow {
	...
	constructor(
		...
		//#region create browser window
		{
			...
			if (useBundledTabs()) {
				// Share one native window across workspaces: the facade looks
				// like a `BrowserWindow` but is backed by a `WebContentsView`.
				this._win = BundleHost.getInstance(options).createTab(options);
			} else {
