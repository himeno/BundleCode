...
import { basename, dirname, join, normalize, posix } from '../../../base/common/path.js';
...
import { BundleHost } from './bundleHost.js';
...
/**
 * 前の回のセッションから戻ってきたリモートを、開く一覧から抜く。
 *
 * **リモートは起動時に開かない。行として残し、行った時に繋ぐ。**
 * 前の回に開いていた分をそのまま復元すると、リモートのプロジェクトの数だけ ssh の
 * 接続が一斉に始まる。繋がらなければそれぞれが独立に再試行を繰り返し、断ると空の
 * ウィンドウが残って、ストリップにはその分だけ行が増える——起動しただけで片付けの
 * 仕事が生まれる。
 *
 * **抜いても行は消えない。** 一覧はプロジェクトの一覧で、開いているかどうかとは別に
 * `bundlecode.json` が覚えている。閉じた行として並び、押せばそこで繋ぎに行く。
 * ローカルは今までどおり開く：待ち時間も失敗も無いので、遅らせて得るものが無い。
 *
 * **落とすのは復元の分だけ。** `bcodex --folder-uri vscode-remote://…` で名指しされた
 * ものまで黙って開かないのは、頼まれたことをしないのと同じ。
 *
 * 判断は URI の scheme で行う。`remoteAuthority` は空のウィンドウにも付くので、
 * 「どこかのホストに繋ぐつもりの窓」と「向こうのフォルダーを開く窓」を分けられない。
 * 見たいのは後者だけで、それは必ず `vscode-remote://` を持つ。
 */
function withoutRestoredRemotes(pathsToOpen: IPathToOpen[]): IPathToOpen[] {
	return pathsToOpen.filter(path => {
		const uri = isSingleFolderWorkspacePathToOpen(path) ? path.workspace.uri
			: isWorkspacePathToOpen(path) ? path.workspace.configPath
				: undefined;
...
export class WindowsMainService extends Disposable implements IWindowsMainService {
	...
	constructor(
		...
		// The gear opens the settings file itself rather than a settings UI: the
		// strip's own configuration is meant to be edited as JSON.
		BundleHost.setSettingsOpener(() => this.open({
			context: OpenContext.API,
			cli: this.environmentMainService.args,
			urisToOpen: [{ fileUri: this.userDataProfilesMainService.defaultProfile.settingsResource }],
			forceReuseWindow: true
		}));

		BundleHost.setFileOpener(path => this.open({
			context: OpenContext.API,
			cli: this.environmentMainService.args,
			urisToOpen: [{ fileUri: URI.file(path) }],
			forceReuseWindow: true
		}));

		// An empty window rather than a folder: the strip knows which host was
		// asked for and nothing more. The remote extension resolves the authority
		// from here, the same way it does for `Connect to Host in New Window`.
		// Once a folder is opened on the far side, its `vscode-remote://` address
		// is an ordinary project key and comes back through `setProjectOpener`.
		BundleHost.setRemoteOpener(host => {
			this.openEmptyWindow({ context: OpenContext.API }, { remoteAuthority: `ssh-remote+${host}` });
		});

		// The layout lives next to settings.json as plain JSON. It is the file the
		// user already edits by hand, it needs no permission the settings file
		// does not already have, and it can be read or repaired without going
		// through a database.
		const layoutPath = join(dirname(this.userDataProfilesMainService.defaultProfile.settingsResource.fsPath), 'bundlecode.json');

		BundleHost.setStateStore({
			read: () => {
				try {
					return JSON.parse(fs.readFileSync(layoutPath, 'utf8'));
				} catch (error) {
					// Either nothing has been written yet or the file is unusable.
					// Falling back to the old location keeps a list that predates
					// the move, and beats showing an empty strip.
					this.logService.trace('[BundleHost] reading the layout file failed, falling back to stored state', error);

					return stateService.getItem('bundleHost.layout');
				}
			},
			write: layout => {
				// Through a temporary file: a half-written layout reads as an empty
				// strip, which is indistinguishable from having lost the list.
				const temporaryPath = `${layoutPath}.tmp`;
				try {
					fs.writeFileSync(temporaryPath, JSON.stringify(layout, undefined, '\t'));
					fs.renameSync(temporaryPath, layoutPath);
				} catch (error) {
					this.logService.error('[BundleHost] could not write the layout', error);
				}
			}
		});

		// Lets the strip open a remembered project without reaching into services
		// itself. Which kind of opening it is has to be decided here, because a
		// workspace file and a folder are not interchangeable.
		// A tab's id is its window's id: the facade reports the tab's own.
		BundleHost.setTabReloader(tabId => this.getWindowById(tabId)?.reload());

		BundleHost.setProjectOpener(key => {
			const uri = URI.parse(key);
			const toOpen = hasWorkspaceFileExtension(uri) || uri.path.endsWith('/workspace.json')
				? { workspaceUri: uri }
				: { folderUri: uri };
	...
	private async getPathsToOpen(openConfig: IOpenConfiguration): Promise<IPathToOpen[]> {
		...
		// Extract paths: from previous session
			...
			// 起動時だけ。ここを通らない復元（`--reuse-window` 等）は素のまま。
			if (openConfig.initialStartup) {
				const kept = withoutRestoredRemotes(pathsToOpen);
				if (kept.length !== pathsToOpen.length) {
					this.logService.trace(`[BundleHost] deferring ${pathsToOpen.length - kept.length} remote window(s) until their project is opened`);
					pathsToOpen = kept;
				}
			}
	...
	private async openInBrowserWindow(options: IOpenBrowserWindowOptions): Promise<ICodeWindow> {
		...
		// Let the bundled tab strip recognise this project across restarts.
		// Whether this window is coming back by itself decides if it counts as
		// having been visited: the strip can order by recency, and a restore is
		// not a visit.
		const workspace = configuration.workspace;
		BundleHost.setTabWorkspace(window.id, isSingleFolderWorkspaceIdentifier(workspace)
			? workspace.uri.toString()
			: isWorkspaceIdentifier(workspace) ? workspace.configPath.toString() : undefined,
			!!options.initialStartup);
	...
	getWindowByWebContents(webContents: WebContents): ICodeWindow | undefined {
		...
		// Resolved by scanning rather than through `BrowserWindow.fromWebContents`
		// because several workspaces can share one native window, which makes the
		// web contents to window mapping ambiguous.
